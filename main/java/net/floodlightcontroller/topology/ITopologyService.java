...
public interface ITopologyService extends IFloodlightService  {
	 ...
	 * @return
	 */
	public boolean isIncomingBroadcastAllowed(DatapathId sw, OFPort portId);  //查看指定交换机端口是否允许广播
	 ...
	 */
	public Set<OFPort> getPorts(DatapathId sw);  //查看sw的所有使能(&non quarantined)的port
	 ...
	 * @return
	 */
	public DatapathId getOpenflowDomainId(DatapathId switchId);  //查看sw所在的domain id(即该domain中最小的dpid)
	 ...
	 */
	public boolean inSameOpenflowDomain(DatapathId switch1, DatapathId switch2);  //判断sw1和sw2是否在同一domian
	...
	//获取(src,srcPort)->(dst,dstPort)路径上的下一跳(sw,port)
	/**
	 * If trying to route a packet ingress a source switch+port to a destination
	 * switch+port, retrieve the ingress destination switch+port leading to the destination.
	 * @param src
